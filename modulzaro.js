/*
    ...
    Ebben a dolgozatban egy webshopféleséget kell építened, először lokális, majd szerverről érkező adatokat felhasználva.
    Kövesd a feladatokban található utasításokat!

    Kérlek minden feladatot figyelmesen olvass el, és a feladat szövegében megadott feladatot oldd meg!

    A feladatokat alapvetően bárhogyan megoldhatod, de a teljesértékű megoldásokhoz a legtöbb esetben szükséges használni azokat a metódusokat,
    amelyeket ebben a modulban tanultunk.
...
const printAllProducts = (array) => array.forEach((element) => console.log(element));
...
const printCheaperThan = (array, maxPrice) => console.log(array.filter((element) => element.price < maxPrice));
// printCheaperThan(shopData, 150);
...
const getCheapestProduct = (array) =>
	console.log(array.reduce((min, element) => (min > element.price ? element.price : min), array[0].price));
// getCheapestProduct(shopData);

// 4. Feladat (15)

// Készíts egy render nevű függvényt, ami egy termékeket tartalmazó tömböt fogad el paraméterként és ha meghívjuk
// kirajzolja az összes terméket egy HTML fájlba!
...
const getUniqueCategories = (array) => {
	const categories = array.map((product) => product.category);
...
const addNewButton = (category) => {
	const newButton = document.createElement("button");
	newButton.className = "category-btn";
	newButton.innerText = category;
	newButton.onclick = () => newButton.classList.toggle("active");
	categoriesContainer.append(newButton);
};

const drawCategories = (categoriesArray) => {
...
const connectCardsToButtonEvents = () => {
	const categoryButtons = [...document.getElementsByClassName("category-btn")];
	categoryButtons.forEach((button) => {
		button.addEventListener("click", renderActiveCards);
	});
};
...
// Készíts egy renderFromAPI nevű függvényt, ami a shopData tömb helyett a következő API-ról tölti le a termékek adatait:
// https://fakestoreapi.com/products

// Miután megvannak a termékek adatai, hívd meg az előzőekben elkészített render,
// getUniqueCategories és drawCategories metódusokat a letöltött termékekkel!
...
const renderFromAPI = async () => {
	try {
		...
		const shopDataAPI = await response.json();

		// throw new Error("Hiba az adatok letöltésénél !");

		products.innerHTML = "";
		categoriesContainer.innerHTML = "";

		render(shopDataAPI);
		drawCategories(getUniqueCategories(shopDataAPI));
		connectCardsToButtonEvents();
	} catch (error) {
